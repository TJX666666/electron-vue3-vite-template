...
export class Window {
  ...
  createView(options:any){
    // console.log(this.getWindow(1));
    // this.getWindow(1).setBrowserView(this.getWindow(1))
    // this.getWindow(1).setBounds({ x: 0, y: 0, width: 300, height: 300 })
    // this.getWindow(1).webContents.loadURL(`http://${process.env["VITE_DEV_SERVER_HOST"]}:${process.env["VITE_DEV_SERVER_PORT"]}${options.route}`)
  } 
  ...
  listen() {
    ...
    ipcMain.on('view-new',(args)=>this.createView(args))
  }
}
